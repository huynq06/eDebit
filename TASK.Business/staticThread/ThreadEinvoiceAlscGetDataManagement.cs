...
{
    public class ThreadEinvoiceAlscGetDataManagement
    {
        #region singleton pattern

        List<WorkingBaseTimer> ThreadList;

        private ThreadEinvoiceAlscGetDataManagement()
        {
            ThreadList = new List<WorkingBaseTimer>();
            WakeupTimer wakeup = new WakeupTimer();
            ThreadList.Add(wakeup);
            ThreadList.Add(new ThreadCreadIssueInvoiceHermes());
            ThreadList.Add(new ThreadCreatInvoiceManagement());

        }

        private static object _lock = new object();
        private static ThreadEinvoiceAlscGetDataManagement _instance;
        public static ThreadEinvoiceAlscGetDataManagement Instance
        {
            get
            {
                if (_instance == null)
                {
                    lock (_lock)
                    {
                        _instance = new ThreadEinvoiceAlscGetDataManagement();
                    }
                }
                return _instance;
            }
